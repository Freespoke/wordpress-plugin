...
class PublisherTest extends TestCase
{
    ...
    public function testOnStatusTransitionSubmitsCustomPostType(): void
    {
        $settings = \Mockery::mock(Settings::class);
        $settings->allows('getNoticeEmails')->andReturn([]);
        $settings->allows('getPostTypes')->andReturn(['post', 'event']);
        $settings->allows('getContentMetaFieldKeys')->andReturn([]);
        $publisher = new Publisher($this->factory, $this->postMeta, $settings);

        $post = $this->makePost(1, 'publish', 'event');
        $result = new IndexResult();
        $result->job_id = 'job-e';
        $result->workflow_id = 'wf-e';
        ...
        $publisher->onStatusTransition('publish', 'future', $post);
    }
