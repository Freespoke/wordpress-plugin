...
class SettingsTest extends TestCase
{
    ...
    public function testGetTestModeDefault(): void
    {
        Functions\expect('get_option')
            ->once()
            ->with('freespoke_test_mode', false)
            ->andReturn(false);

        $settings = new Settings();
        $this->assertFalse($settings->getTestMode());
    }

    public function testGetTestModeFromOption(): void
    {
        Functions\expect('get_option')
            ->once()
            ->with('freespoke_test_mode', false)
        ...
        $settings = new Settings();
        $this->assertTrue($settings->getTestMode());
    }

    public function testGetTestModeFromOverrideTrue(): void
    {
        $settings = new Settings(['test_mode' => true]);
        $this->assertTrue($settings->getTestMode());
    }

    public function testGetTestModeFromOverrideFalse(): void
    {
        $settings = new Settings(['test_mode' => false]);
        $this->assertFalse($settings->getTestMode());
    }

    public function testGetTestModeCoercesTruthyOverride(): void
    {
        $settings = new Settings(['test_mode' => '1']);
        $this->assertTrue($settings->getTestMode());
    }

    public function testIsTestModeLocked(): void
    {
        $this->assertTrue((new Settings(['test_mode' => true]))->isTestModeLocked());
        $this->assertTrue((new Settings(['test_mode' => false]))->isTestModeLocked());
        $this->assertFalse((new Settings())->isTestModeLocked());
    }

    public function testSaveTestModeTrue(): void
    {
        Functions\expect('update_option')
            ->once()
            ->with('freespoke_test_mode', true);
        Functions\expect('update_option')->zeroOrMoreTimes();

        $settings = new Settings();
        $settings->save(['test_mode' => true]);
    }

    public function testSaveTestModeFalse(): void
    {
        Functions\expect('update_option')
            ->once()
            ->with('freespoke_test_mode', false);
    ...
    public function testSaveUpdatesUnlockedFields(): void
    {
        ...
        Functions\expect('update_option')->times(7);
    ...
    {
        // post_types and test_mode always save when unlocked (defaults).
        Functions\expect('update_option')
            ->once()
