...
class AdminTest extends TestCase
{
    ...
    public function testRenderTestModeNoticeRendersWhenEnabled(): void
    {
        $this->settings->expects('getTestMode')->once()->andReturn(true);
        Functions\expect('admin_url')
            ->once()
            ->with('tools.php?page=freespoke-publisher')
            ->andReturn('https://example.test/wp-admin/tools.php?page=freespoke-publisher');
        Functions\expect('esc_url')->andReturnFirstArg();
        Functions\expect('esc_html_e')->andReturnUsing(function ($s) { echo $s; });

        ob_start();
        $this->admin->renderTestModeNotice();
        $output = ob_get_clean();

        $this->assertStringContainsString('notice notice-warning', $output);
        $this->assertStringContainsString('test mode', $output);
        $this->assertStringContainsString('tools.php?page=freespoke-publisher', $output);
    }

    public function testRenderTestModeNoticeIsSilentWhenDisabled(): void
    {
        $this->settings->expects('getTestMode')->once()->andReturn(false);
