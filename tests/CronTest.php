...
class CronTest extends TestCase
{
    ...
    public function testPollFailedJobSetsErrorAndNotifies(): void
    {
        ...
        // No status code → defaults to OK (0), which is treated as retryable.
        ...
        $this->postMeta->expects('setError')
            ->once()
            ->with(42, 'job_failed', \Mockery::type('string'), true);
        $this->postMeta->expects('clearJobId')->once()->with(42);
    ...
    public function testPollFailedJobWithPermanentCodeMarksNoRetry(): void
    {
        $this->publisher->expects('getEpoch')->andReturn(new \WP_Error('fail', 'fail'));

        $this->postMeta->expects('getPostsWithPendingJobs')->andReturn([42]);
        $this->postMeta->expects('getJobId')->with(42)->andReturn('job-bad');

        $status = new IndexStatusResult();
        $status->status = 'JOB_STATUS_FAILED';
        // gRPC code 9 = FAILED_PRECONDITION — won't be fixed by retrying.
        $status->error = ['code' => 9, 'message' => 'language not allowed'];
        $this->client->expects('getIndexStatus')->with('job-bad')->andReturn($status);

        $this->postMeta->expects('setError')
            ->once()
            ->with(42, 'job_failed', \Mockery::type('string'), false);
        $this->postMeta->expects('clearJobId')->once()->with(42);
        $this->publisher->expects('notifyFailure')->once();

        $this->cron->run();
    }

    public function testPollFailedJobWithTransientCodeRetryable(): void
    {
        $this->publisher->expects('getEpoch')->andReturn(new \WP_Error('fail', 'fail'));

        $this->postMeta->expects('getPostsWithPendingJobs')->andReturn([42]);
        $this->postMeta->expects('getJobId')->with(42)->andReturn('job-flake');

        $status = new IndexStatusResult();
        $status->status = 'JOB_STATUS_FAILED';
        // gRPC code 14 = UNAVAILABLE — transient.
        $status->error = ['code' => 14, 'message' => 'service unavailable'];
        $this->client->expects('getIndexStatus')->with('job-flake')->andReturn($status);
    ...
    public function testPollPendingJobsPersistsDocId(): void
    {
        $this->publisher->expects('getEpoch')->andReturn(new \WP_Error('fail', 'fail'));

        $this->postMeta->expects('getPostsWithPendingJobs')->andReturn([42]);
        $this->postMeta->expects('getJobId')->with(42)->andReturn('job-1');

        $status = new IndexStatusResult();
        $status->job_id = 'job-1';
        $status->status = 'JOB_STATUS_COMPLETE';
        $status->doc_id = 'doc-xyz';
        $this->client->expects('getIndexStatus')->with('job-1')->andReturn($status);

        $this->postMeta->expects('setDocId')->once()->with(42, 'doc-xyz');
        $this->postMeta->expects('clearJobId')->once()->with(42);
        $this->postMeta->expects('clearError')->once()->with(42);
