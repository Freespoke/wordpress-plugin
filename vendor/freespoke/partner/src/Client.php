...
class Client
{
    ...
    public function getIndexStatus(string $job_id): ?IndexStatusResult
    {
        ...
        $result->doc_id = $type === 'type.googleapis.com/google.protobuf.StringValue' && is_string($value) && $value !== '' ? $value : null;
        return $result;
    }
    /**
     * Request deletion of a previously-indexed URL.
     *
     * @param string $url The URL to delete.
     * @param string|null $id Optional IndexItem id (40-char hex) returned by index().
     * @return DeleteResult|null
     * @throws \GuzzleHttp\Exception\GuzzleException
     * @throws \JsonException
     */
    public function delete(string $url, ?string $id = null): ?DeleteResult
    {
        $query = ['url' => $url];
        if ($id !== null && $id !== '') {
            $query['id'] = $id;
        }
        $data = $this->requestJson('DELETE', 'v1/content', ['query' => $query]);
        $errorMessage = $data['errorMessage'] ?? $data['error_message'] ?? null;
        if (is_string($errorMessage) && $errorMessage !== '') {
            return null;
        }
    ...
    {
        $payload = ['url' => $article->url ?? '', 'title' => $article->title ?? '', 'description' => $article->description ?? '', 'content' => $article->content ?? '', 'authors' => $this->formatAuthors($article->getAuthors()), 'keywords' => $article->keywords ?? [], 'test_mode' => $article->test_mode];
