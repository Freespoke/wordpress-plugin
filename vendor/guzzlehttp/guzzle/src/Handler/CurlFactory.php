...
namespace FreespokeDeps\GuzzleHttp\Handler;

use FreespokeDeps\GuzzleHttp\Exception\ConnectException;
use FreespokeDeps\GuzzleHttp\Exception\RequestException;
use FreespokeDeps\GuzzleHttp\Promise as P;
use FreespokeDeps\GuzzleHttp\Promise\FulfilledPromise;
use FreespokeDeps\GuzzleHttp\Promise\PromiseInterface;
use FreespokeDeps\GuzzleHttp\Psr7\LazyOpenStream;
use FreespokeDeps\GuzzleHttp\TransferStats;
use FreespokeDeps\GuzzleHttp\Utils;
use FreespokeDeps\Psr\Http\Message\RequestInterface;
use FreespokeDeps\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    public function create(RequestInterface $request, array $options): EasyHandle
    {
        ...
        if ('' === $protocolVersion) {
            $protocolVersion = '1.1';
            $request = \FreespokeDeps\GuzzleHttp\Psr7\Utils::modifyRequest($request, ['version' => $protocolVersion]);
        }
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \FreespokeDeps\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \FreespokeDeps\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \FreespokeDeps\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \FreespokeDeps\GuzzleHttp\Psr7\Utils::streamFor($sink);
