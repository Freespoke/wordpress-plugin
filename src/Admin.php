...
class Admin
{
    ...
    public function renderPage(): void
    {
                        ...
                        <td>
                            <fieldset>
                                <label>
                                    ...
                                    <?php 
        esc_html_e('Posts', 'freespoke-widget');
        ?>
                                </label>
                                <br />
                                <label for="freespoke_include_pages">
                                    <input name="include_pages" id="freespoke_include_pages" type="checkbox" value="1"
                                           <?php 
        checked($this->settings->isIncludePages());
        ?>
                                           <?php 
        disabled($this->settings->isIncludePagesLocked());
        ?> />
                                    <?php 
        esc_html_e('Pages', 'freespoke-widget');
        ?>
                                </label>
                                <p class="description">
                                    <?php 
        ...
        ?>
                                    <?php 
        if ($this->settings->isIncludePagesLocked()) {
            ?>
                                        <br /><?php 
            esc_html_e('Set via FREESPOKE_INCLUDE_PAGES constant.', 'freespoke-widget');
            ?>
                                    <?php 
        }
        ?>
    ...
    private function handleFormSubmit(): string
    {
        ...
        $this->settings->save(['auth_mode' => isset($_POST['auth_mode']) ? wp_unslash($_POST['auth_mode']) : null, 'client_id' => isset($_POST['client_id']) ? wp_unslash($_POST['client_id']) : null, 'client_secret' => isset($_POST['client_secret']) ? wp_unslash($_POST['client_secret']) : null, 'api_key' => isset($_POST['api_key']) ? wp_unslash($_POST['api_key']) : null, 'notice_emails' => isset($_POST['notice_emails']) ? wp_unslash($_POST['notice_emails']) : null, 'include_pages' => !empty($_POST['include_pages'])]);
