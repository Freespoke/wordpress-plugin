...
class Admin
{
    ...
    public function renderPage(): void
    {
        ...
        ?>
                    ...
                    <tr>
                        <th scope="row">
                            <?php 
        esc_html_e('Content Meta Fields', 'freespoke-widget');
        ?>
                        </th>
                        <td>
                            <?php 
        $metaFields = $this->settings->getContentMetaFields();
        $metaLocked = $this->settings->isContentMetaFieldsLocked();
        ?>
                            <style>
                                .freespoke-meta-fields { max-width: 640px; }
                                .freespoke-meta-row {
                                    display: flex;
                                    gap: 8px;
                                    align-items: center;
                                ...
                                .freespoke-meta-row .regular-text {
                                    flex: 1;
                                    min-width: 0;
                                }
                                .freespoke-meta-remove {
                                    flex: 0 0 auto;
                            ...
                            <div id="freespoke-meta-fields" class="freespoke-meta-fields">
                                <?php 
        $rowIndex = 0;
        $renderRow = function (int $i, string $fieldName, string $description) use ($metaLocked): void {
            ?>
                                    <div class="freespoke-meta-row">
                                        <input type="text" class="regular-text"
                                               placeholder="<?php 
            esc_attr_e('Meta field name', 'freespoke-widget');
            ?>"
                                               aria-label="<?php 
            esc_attr_e('Meta field name', 'freespoke-widget');
            ?>"
                                               name="content_meta_fields[<?php 
            echo $i;
            ?>][field_name]"
                                               value="<?php 
            ...
            ?> />
                                        <input type="text" class="regular-text"
                                               placeholder="<?php 
            esc_attr_e('Description (for your reference)', 'freespoke-widget');
            ?>"
                                               aria-label="<?php 
            esc_attr_e('Description', 'freespoke-widget');
            ?>"
                                               name="content_meta_fields[<?php 
            echo $i;
            ?>][description]"
            ...
            disabled($metaLocked);
            ?> />
                                        <button type="button" class="button-link button-link-delete freespoke-meta-remove"
                                                aria-label="<?php 
            esc_attr_e('Remove row', 'freespoke-widget');
            ?>"
                                                <?php 
            disabled($metaLocked);
            ?>>&times;</button>
        ...
        foreach ($metaFields as $fieldName => $description) {
            $renderRow($rowIndex, $fieldName, $description);
            $rowIndex++;
        }
        $renderRow($rowIndex, '', '');
        ?>
                            ...
                            <p>
                                <button type="button" id="freespoke-meta-add" class="button"
                                        <?php 
        disabled($metaLocked);
        ?>>
                                    <?php 
        esc_html_e('+ Add row', 'freespoke-widget');
        ?>
                                </button>
                            </p>
                            <p class="description">
                                <?php 
        esc_html_e('Post-meta keys whose values will be appended to the indexed content. Use this when your content lives in custom fields rather than the main editor. Descriptions are labels for your own reference and are not sent to Freespoke.', 'freespoke-widget');
        ?>
                                <?php 
        if ($metaLocked) {
            ?>
                                    <br /><?php 
            echo esc_html(sprintf(__('Set via %s constant.', 'freespoke-widget'), $this->settings->getContentMetaFieldsLockSource()));
            ?>
                                <?php 
        }
        ?>
        ...
        if (!$metaLocked) {
            ?>
                                <script>
                                (function () {
                                    var container = document.getElementById('freespoke-meta-fields');
                                    var addBtn = document.getElementById('freespoke-meta-add');
                                    if (!container || !addBtn) { return; }

                                    function nextIndex() {
                                        var inputs = container.querySelectorAll('input[name^="content_meta_fields["]');
                                        var max = -1;
                                        inputs.forEach(function (el) {
                                            var m = el.name.match(/^content_meta_fields\[(\d+)\]/);
                                    ...
                                    addBtn.addEventListener('click', function () {
                                        ...
                                        row.className = 'freespoke-meta-row';
                                        row.innerHTML =
                                            '<input type="text" class="regular-text" placeholder="Meta field name" aria-label="Meta field name" name="content_meta_fields[' + i + '][field_name]" value="" />' +
                                            '<input type="text" class="regular-text" placeholder="Description (for your reference)" aria-label="Description" name="content_meta_fields[' + i + '][description]" value="" />' +
                                            '<button type="button" class="button-link button-link-delete freespoke-meta-remove" aria-label="Remove row">&times;</button>';
                                        container.appendChild(row);
                                    });

                                    container.addEventListener('click', function (e) {
                                        var btn = e.target.closest('.freespoke-meta-remove');
                                        if (!btn) { return; }
                                        var row = btn.closest('.freespoke-meta-row');
                                        if (row) { row.remove(); }
                                    });
                                })();
                                </script>
        ...
        ?>
                    ...
                    <tr>
                        <th scope="row">
                            <label for="freespoke_test_mode"><?php 
        esc_html_e('Test Mode', 'freespoke-widget');
        ?></label>
                        </th>
                        <td>
                            <label>
                                <input name="test_mode" id="freespoke_test_mode" type="checkbox" value="1"
                                       <?php 
        checked($this->settings->getTestMode());
        ?>
                                       <?php 
        disabled($this->settings->isTestModeLocked());
        ?> />
                                <?php 
        esc_html_e('Enable test mode', 'freespoke-widget');
        ?>
                            </label>
                            <p class="description"><?php 
        esc_html_e('When enabled, submissions to Freespoke are validated and processed end-to-end but the resulting content is not stored. Use this to verify the integration without publishing real content.', 'freespoke-widget');
        ?></p>
                            <?php 
        if ($this->settings->isTestModeLocked()) {
            ?>
                                <p class="description"><?php 
            esc_html_e('Set via FREESPOKE_TEST_MODE constant.', 'freespoke-widget');
            ?></p>
                            <?php 
        }
        ?>
    ...
    public function renderTestModeNotice(): void
    {
        if (!$this->settings->getTestMode()) {
            return;
        }
        $settingsUrl = admin_url('tools.php?page=freespoke-publisher');
        ?>
        <div class="notice notice-warning">
            <p>
                <strong><?php 
        esc_html_e('Freespoke Publisher is in test mode.', 'freespoke-widget');
        ?></strong>
                <?php 
        esc_html_e('Submissions are validated but not stored by Freespoke.', 'freespoke-widget');
        ?>
                <a href="<?php 
        echo esc_url($settingsUrl);
        ?>"><?php 
        esc_html_e('Open settings', 'freespoke-widget');
        ?></a>
            </p>
        </div>
        <?php 
