...
class Settings
{
    ...
    public function isIncludePages(): bool
    {
        if (isset($this->overrides['include_pages'])) {
            return filter_var($this->overrides['include_pages'], \FILTER_VALIDATE_BOOLEAN);
        }
        return filter_var(get_option('freespoke_include_pages', \false), \FILTER_VALIDATE_BOOLEAN);
    }
    public function isIncludePagesLocked(): bool
    {
        return isset($this->overrides['include_pages']);
    }
    /**
     * @return string[]
     */
    ...
    public function save(array $input): void
    {
        ...
        if (!$this->isIncludePagesLocked()) {
            update_option('freespoke_include_pages', !empty($input['include_pages']));
        }
