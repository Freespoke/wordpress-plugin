...
class Cron
{
    ...
    private function pollPendingJobs(): void
    {
        ...
        foreach ($postIds as $postId) {
            ...
            if ($status->status === 'JOB_STATUS_COMPLETE') {
                if (is_string($status->doc_id) && $status->doc_id !== '') {
                    $this->postMeta->setDocId($postId, $status->doc_id);
                }
                ...
                $retryable = $this->isRetryableStatusCode((int) ($status->error['code'] ?? 0));
                $this->postMeta->setError($postId, 'job_failed', $errorMsg, $retryable);
                $this->postMeta->clearJobId($postId);
    ...
    /**
     * Classify a google.rpc.Code value as retryable. Codes that indicate
     * the request itself is malformed or violates a precondition will not
     * be fixed by re-running — those posts wait for an edit or for the
     * partner epoch to advance past the failure. Everything else
     * (transport, transient, or unknown) is retried on the cron schedule.
     */
    private function isRetryableStatusCode(int $code): bool
    {
        ...
        $permanent = [3, 5, 6, 7, 9, 11, 12, 16];
        return !in_array($code, $permanent, \true);
    }
}
