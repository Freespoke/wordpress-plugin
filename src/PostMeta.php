...
class PostMeta
{
    ...
    public function getPostsWithPendingJobs(int $limit = 50): array
    {
        $query = new \WP_Query(['post_type' => 'post', 'post_status' => 'publish', 'posts_per_page' => $limit, 'fields' => 'ids', 'meta_query' => [['key' => self::KEY_JOB_ID, 'compare' => 'EXISTS']]]);
        return $query->posts;
    }
    ...
    public function getPostsWithErrors(int $limit = 50): array
    {
        $posts = get_posts(['post_type' => 'post', 'post_status' => ['publish', 'future', 'draft', 'pending', 'private'], 'posts_per_page' => $limit, 'meta_query' => [['key' => self::KEY_ERROR, 'compare' => 'EXISTS']]]);
        $results = [];
    ...
    public function getPostsNeedingIndex(int $epoch, int $limit = 50): array
    {
        $query = new \WP_Query(['post_type' => 'post', 'post_status' => 'publish', 'posts_per_page' => $limit, 'fields' => 'ids', 'meta_query' => ['relation' => 'OR', ['key' => self::KEY_SUBMIT_TIME, 'compare' => 'NOT EXISTS'], ['key' => self::KEY_SUBMIT_TIME, 'value' => $epoch, 'type' => 'NUMERIC', 'compare' => '<']]]);
        return $query->posts;
    }
}
