...
class PostMeta
{
    ...
    public function setError(int $postId, string $code, string $message, bool $retryable = \true): void
    {
        ...
        if ($retryable) {
            ...
            // The value is the failure time, not a boolean: a permanent
            // failure is only permanent for the epoch it happened under.
            // getPostsNeedingIndex retries the post once the partner epoch
            // advances past this timestamp.
            update_post_meta($postId, self::KEY_NO_RETRY, (string) time());
        }
    }
    ...
    {
        $query = new \WP_Query(['post_type' => $postType, 'post_status' => 'publish', 'posts_per_page' => $limit, 'fields' => 'ids', 'meta_query' => ['relation' => 'AND', [
            // A permanent failure parks the post only for the epoch
            // it failed under: when the partner epoch advances past
            // the failure time, the verdict is stale (server or
            // config may have changed) and the post is retried.
            // Legacy installs stored '1' here, which compares below
            // any real epoch — failures from before this version
            // retry automatically, no operator action needed.
            'relation' => 'OR',
            ['key' => self::KEY_NO_RETRY, 'compare' => 'NOT EXISTS'],
            ['key' => self::KEY_NO_RETRY, 'value' => $epoch, 'type' => 'NUMERIC', 'compare' => '<'],
        ], ['relation' => 'OR', ['key' => self::KEY_SUBMIT_TIME, 'compare' => 'NOT EXISTS'], ['key' => self::KEY_SUBMIT_TIME, 'value' => $epoch, 'type' => 'NUMERIC', 'compare' => '<']]]]);
        return $query->posts;
    }
}
